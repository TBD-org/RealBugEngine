...
void ComponentCapsuleCollider::OnCollision(GameObject& collidedWith, float3 contactPoint, float3 normalCollision) {
	for (ComponentScript& scriptComponent : GetOwner().GetComponents<ComponentScript>()) {
		...
		if (script != nullptr) {
			script->OnCollision(collidedWith, contactPoint, normalCollision);
		}
	}
}

float ComponentCapsuleCollider::GetRadius() {
	return radius;
}